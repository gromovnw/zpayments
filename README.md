...
При старте сервера плагин сам проверяет GitHub-релизы и при наличии новой версии кладёт джарник в `plugins/update/` — применится на следующем перезапуске. Отключается в `config.yml` (`update.enabled: false`).
